...
@section('content')
    ...
    <div class="storage-dashboard-grid">
        <section class="admin-panel"><header class="admin-panel__header"><div><h2>Stok perlu perhatian</h2><p>Jumlah total sama atau di bawah minimum.</p></div><a class="admin-link" href="{{ route('admin.storage-items.index') }}">Lihat stok</a></header>
            @if($lowStockItems->isEmpty())<div class="admin-empty admin-empty--compact"><h2>Semua stok aman</h2><p>Belum ada consumable di bawah batas minimum.</p></div>@else<x-ui.table class="admin-table-wrap"><thead><tr><th>Barang</th><th>Stok</th><th>Status</th></tr></thead><tbody>@foreach($lowStockItems as $item)<tr><td><a class="admin-link" href="{{ route('admin.storage-items.show', $item) }}">{{ $item->name }}</a><small>{{ $item->code }}</small></td><td><strong>{{ format_quantity($item->stocks_sum_quantity ?? 0) }} {{ $item->unit }}</strong><small>Minimum {{ format_quantity($item->minimum_stock) }}</small></td><td><x-admin.status-badge :status="(float)($item->stocks_sum_quantity ?? 0) <= 0 ? 'inactive' : 'pending'">{{ (float)($item->stocks_sum_quantity ?? 0) <= 0 ? 'Habis' : 'Menipis' }}</x-admin.status-badge></td></tr>@endforeach</tbody></x-ui.table>@endif
        </section>
