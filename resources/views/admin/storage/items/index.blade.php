...
@section('content')
    ...
    <form class="admin-filter" method="GET"><label class="admin-field"><span>Cari barang</span><input type="search" name="search" value="{{ request('search') }}" placeholder="Kode atau nama barang"></label><label class="admin-field"><span>Status</span><select name="status"><option value="">Semua status</option><option value="active" @selected(request('status')==='active')>Aktif</option><option value="inactive" @selected(request('status')==='inactive')>Nonaktif</option></select></label><div class="admin-actions"><button class="button button--primary admin-button">Terapkan</button><a class="button button--outline admin-button" href="{{ route('admin.storage-items.index') }}">Reset</a></div></form>
    <section class="admin-panel"><header class="admin-panel__header"><div><h2>Daftar consumable</h2><p>{{ $items->total() }} jenis barang ditemukan.</p></div></header>
        @if($items->isEmpty())<div class="admin-empty"><span><x-ui.icon name="storage" /></span><h2>Belum ada consumable</h2><p>Buat master barang, lalu tambahkan stok melalui Penerimaan Barang.</p></div>@else
            <x-ui.table class="admin-table-wrap"><thead><tr><th>Barang</th><th>Satuan</th><th>Stok total</th><th>Minimum</th><th>Status</th><th>Aksi</th></tr></thead><tbody>
            @foreach($items as $item)@php($total=(float)($item->stocks_sum_quantity ?? 0))<tr>
                <td><strong>{{ $item->name }}</strong><small>{{ $item->code }}</small></td><td><strong>{{ $item->unit->symbol }}</strong><small>{{ $item->unit->name ?: 'Tanpa nama lengkap' }}</small></td>
                <td><strong>{{ format_quantity($total) }} {{ $item->unit->symbol }}</strong></td><td>{{ format_quantity($item->minimum_stock) }} {{ $item->unit->symbol }}</td>
